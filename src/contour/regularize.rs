use crate::coord::{EQUILATERAL_TRIANGLE_HEIGHT, Normalization, TernaryPoint, Tolerance};

#[cfg(feature = "cubic-alpha")]
use super::field::CubicGridField;
use super::{
    ContourError, ContourPath, ContourRegularization, RegularTernaryScalarField,
    field::{LocatedValue, locate_linear},
};

pub(crate) enum FieldEvaluator<'a> {
    Linear(&'a RegularTernaryScalarField),
    #[cfg(feature = "cubic-alpha")]
    Cubic(&'a CubicGridField<'a>),
}
impl FieldEvaluator<'_> {
    fn locate(&self, point: TernaryPoint) -> Result<LocatedValue, ContourError> {
        match self {
...
fn redistribute(
    ...
    closed: bool,
    spacing: f64,
) -> Result<Vec<TernaryPoint>, ContourError> {
    if points.len() < 2 {
...
fn project_point(
    mut point: TernaryPoint,
    level: f64,
    options: ContourRegularization,
    evaluator: &FieldEvaluator<'_>,
    mut trace: Option<&mut Vec<f64>>,
) -> Result<TernaryPoint, ContourError> {
    for iteration in 0..options.max_projection_iterations {
        ...
        for _ in 0..24 {
            ...
            let c = 1.0 - a - b;
            let candidate = TernaryPoint::new(a, b, c).validate(
                Normalization::Normalize,
                Tolerance {
                    absolute: options.projection_tolerance.max(1e-12),
                    relative: 1e-12,
                },
            );
...
fn xy(point: TernaryPoint) -> [f64; 2] {
    let [a, _b, c] = point.as_array();
    [c + 0.5 * a, EQUILATERAL_TRIANGLE_HEIGHT * a]
}
fn distance(left: TernaryPoint, right: TernaryPoint) -> f64 {
    let left = xy(left);
    let right = xy(right);
...
mod tests {
    ...
    fn redistribution_uniforms_spacing_preserves_open_endpoints_and_level() {
        ...
        let start = TernaryPoint::new(0.1, 0.5, 0.4);
        let middle = TernaryPoint::new(0.12, 0.4925, 0.3875);
        let end = TernaryPoint::new(0.5, 0.35, 0.15);
        let level = {
    ...
    fn zero_gradient_and_maximum_step_protection_are_controlled() {
        ...
        let start = TernaryPoint::new(0.1, 0.45, 0.45);
    ...
    fn projection_relocates_across_regular_grid_triangle_boundaries() {
        ...
        let blank = RegularTernaryScalarField::new(n, vec![0.0; count]).unwrap();
        let values = (0..count)
            .map(|index| blank.composition_at(index).unwrap().as_array()[0])
            .collect();
        let field = RegularTernaryScalarField::new(n, values).unwrap();
        let evaluator = FieldEvaluator::Linear(&field);
        let start = TernaryPoint::new(0.05, 0.45, 0.50);
    ...
    fn signed_area(points: &[TernaryPoint]) -> f64 {
        (0..points.len())
            .map(|index| {
