...
#[cfg(feature = "cubic-alpha")]
fn combine(vertices: [TernaryPoint; 3], weights: [f64; 3]) -> TernaryPoint {
    let values = vertices.map(TernaryPoint::as_array);
    TernaryPoint::new(
...
mod tests {
    use super::*;
    use crate::contour::{CubicAlphaOptions, RegularTernaryScalarField};
    ...
    fn adaptive_refinement_detects_closed_interior_contour() {
        let field = field(5);
        let mut model = CubicGridField::new(&field, CubicAlphaOptions::default()).unwrap();
