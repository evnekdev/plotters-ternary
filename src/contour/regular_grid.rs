...
impl RegularTernaryScalarField {
    pub fn new(subdivisions: usize, values: Vec<f64>) -> Result<Self, ContourError> {
        if subdivisions == 0 {
            return Err(ContourError::ZeroSubdivisions);
        }
        let expected = Self::vertex_count_for(subdivisions)?;
        if values.len() != expected {
            return Err(ContourError::IncorrectValueCount {
                expected,
                actual: values.len(),
            });
        }
        ...
        {
            return Err(ContourError::NonFiniteValue { index, value });
        }
        Ok(Self {
            subdivisions,
            values,
    ...
    pub fn triangle_count(&self) -> Result<usize, ContourError> {
        self.subdivisions
            ...
            .ok_or(ContourError::AllocationOverflow)
    }
    ...
    pub fn value(&self, id: GridVertexId) -> Result<f64, ContourError> {
        self.values
            .get(id.0)
            .copied()
            .ok_or(ContourError::InvalidVertexIndex {
                index: id.0,
    ...
    pub fn vertex_id(&self, coordinate: LatticeCoordinate) -> Result<GridVertexId, ContourError> {
        ...
        {
            return Err(ContourError::InvalidLatticeCoordinate {
                i: coordinate.i,
                j: coordinate.j,
                k: coordinate.k,
    ...
    pub fn lattice_coordinate(&self, id: GridVertexId) -> Result<LatticeCoordinate, ContourError> {
        if id.0 >= self.values.len() {
            return Err(ContourError::InvalidVertexIndex {
                index: id.0,
                vertex_count: self.values.len(),
            });
        }
    ...
    #[cfg(test)]
    pub(crate) fn unique_edges(&self) -> Result<BTreeSet<GridEdgeKey>, ContourError> {
        let mut edges = BTreeSet::new();
        for triangle in self.triangles()? {
            for [left, right] in [[0, 1], [1, 2], [2, 0]] {
                edges.insert(GridEdgeKey::new(
    ...
    fn vertex_count_for(subdivisions: usize) -> Result<usize, ContourError> {
        subdivisions
            .checked_add(1)
            .and_then(|left| {
                subdivisions
...
mod tests {
    ...
    fn triangle_and_unique_edge_counts_match_regular_lattice_formulae() {
        for n in 1..=12 {
            let field = field(n);
            assert_eq!(field.triangles().unwrap().len(), n * n);
    ...
    fn construction_rejects_invalid_sizes_values_and_overflow() {
        assert!(matches!(
            ...
            Err(ContourError::ZeroSubdivisions)
        ));
        assert!(matches!(
            RegularTernaryScalarField::new(2, vec![0.0; 5]),
            Err(ContourError::IncorrectValueCount {
                expected: 6,
                actual: 5
            })
        ));
        ...
        assert!(matches!(
            ...
            Err(ContourError::NonFiniteValue { index: 3, .. })
        ));
        assert!(matches!(
            RegularTernaryScalarField::new(usize::MAX, Vec::new()),
            Err(ContourError::AllocationOverflow)
        ));
    }
}
