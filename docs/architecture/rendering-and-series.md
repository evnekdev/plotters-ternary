...
Milestone 7 implements backend-independent contours over `RegularTernaryScalarField`. The numerical kernel owns regular-grid indexing, linear interpolation, canonical directed cubic-alpha edge intervals, local implicit fields, adaptive topology, path joining, and optional level-set regularization. It has no Plotters, orientation, viewport, or screen-coordinate dependency.
