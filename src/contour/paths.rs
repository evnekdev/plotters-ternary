...
pub(crate) fn join_segments(
    ...
    let mut nodes: Vec<TernaryPoint> = Vec::new();
...
fn node_for(nodes: &mut Vec<TernaryPoint>, point: TernaryPoint, tolerance: f64) -> usize {
...
fn close(left: TernaryPoint, right: TernaryPoint, tolerance: f64) -> bool {
    left.as_array()
        .into_iter()
        .zip(right.as_array())
        .all(|(a, b)| (a - b).abs() <= tolerance)
}
fn lex_cmp(left: TernaryPoint, right: TernaryPoint) -> std::cmp::Ordering {
...
mod tests {
    use super::*;
    fn p(a: f64, b: f64) -> TernaryPoint {
        TernaryPoint::new(a, b, 1.0 - a - b)
    }
    #[test]
